...
int main() {
    ...
    std::unique_ptr<Component> branch1(new Composite(&op_plus, leaf_1.get(), leaf_2.get()));

    std::unique_ptr<Component> leaf_3(new Leaf(3));
    std::unique_ptr<Component> leaf_4(new Leaf(4));
    std::unique_ptr<Component> branch2(new Composite(&op_minus, leaf_3.get(), leaf_4.get()));

    std::unique_ptr<Component> tree(new Composite(&op_plus, branch1.get(), branch2.get()));
